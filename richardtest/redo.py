...
f4 = [1, 0]
f5 = [1, 0]
curve = ['curved', 'uncurved']
closed = ['closed', 'open']
wrist = ['wrist', 'no wrist']
labels = []
versions = []

for combination in itertools.product(p, f1, f2, f3, f4, f5):
    labels.append(combination)

for combination in itertools.product(curve, closed, wrist):
    versions.append(combination)
...
try:
    ...
    while not saved:
        ...
        if len(images)>0:
            ...
            np.save('dataset/100111/1.npy', img)
            pyplot.imshow(img)
            plt.show()
            record = True
        if record:
